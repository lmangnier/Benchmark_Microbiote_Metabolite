list.Scenarios_100_100_M2M_index = list()
list.Scenarios_100_100_M2M_Microbiotes = list()
list.Scenarios_100_100_M2M_Metabolites = list()
list.Scenarios_100_100_M2M_effects = list()
...
for(rep in 1:1000){
  ...
  index.associated.microbiote = sample(1:100, round(100*round(runif(1,0.03,0.2), 2)))
  index.impacted.metabolite = sample(1:100, round(100*round(runif(1,0.03,0.2), 2)))
  
  
  index.associated.metabolite = sample(c(1:100)[-index.impacted.metabolite], round(100*round(runif(1,0.03,0.2), 2)))
  index.impacted.microbiote = sample(c(1:100)[-index.associated.microbiote], round(100*round(runif(1,0.03,0.2), 2)))
  ...
  mu.metabolites = replicate(100, runif(100,0,1))
  ...
  size.metabolites = runif(100,0.3,1)
  
  
  simulated.microbiotes = matrix(qnbinom(pnorm(multi.norm), mu=mu.microbiotes, size=size.microbiotes), ncol=100,nrow=100)
  simulated.metabolites = matrix(qnbinom(pnorm(multi.norm1), mu=mu.metabolites, size=size.metabolites), ncol=100,nrow=100)
  ...
  list.Scenarios_100_100_M2M_index[[rep]] = list("Microbiote" = lhh, "Metabolite" = lhg)
  list.Scenarios_100_100_M2M_effects[[rep]] = list("Microbiote" = lhh.beta, "Metabolite" = lhg.beta)
  list.Scenarios_100_100_M2M_Microbiotes[[rep]] = simulated.microbiotes
  list.Scenarios_100_100_M2M_Metabolites[[rep]] = simulated.metabolites
